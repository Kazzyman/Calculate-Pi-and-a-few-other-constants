// compile with: "go build -o constants.exe constants.go"
...
import ("fmt"; "time"; "math")
                ...
                var TotalLinesInElapsedNanoSecond float64  // global variables defined outside any function
...
func RicksSwitch() {    // the main if-then-else routine to select a calculation from the menu
    ...
    iterInt64 = 1  // to be used to count iterations in selections that require modulus calculations 
...
p(" ")
    start3 := time.Now()
    iterInt64 = 1          // needed because we will need to do a modulus calc based on iterations 
    iterFloat64 = 0       // needed because a float64 is needed to calculate and display lines of code per second 
    var nextOdd float64
    nextOdd = 1
...
//  for iterInt64 < 9000000000 {  // an optional longer calculation 
...
//
//
                if iterInt64 == 4000000000 {p("... 4 Billion, gets us ten digits  ...")
                p(" ")
                p("   #1 234567890#")
                p("   ",π, "was calculated by the Gottfried Leibniz formula") 
...
fmt.Println("\n\n\n -- You entered '", num, "' to review the derivation of the Pythagorean, which is needed in method #6. We will")
...
//fmt.Println("\\caf\u00e9 , 5*5 is 5\u00b2 , squareroot symbol is \u221A") // Prints \café
//fmt.Println("\u00b0 is degree symbol")
...
//
//
                p("  ... working ... on Billions more iterations ...\n\n")
            }
//
//
//
            if iterInt64 == 600000000 {
                p("  ... working ... did another Two Hundred Thousand iterations ... working ...\n")
                t := time.Now()
                elapsed := t.Sub(start1)
                    p(elapsed)
                    fmt.Println(" ")
            ...
            if iterInt64 == 800000000 {
                 p("  ... working ... did yet another Two Hundred Thousand iterations ... working ...\n")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed, " \n")
            }
...
//
//
            p("  ... working ... on another Billion iterations ...\n\n")
        }
...
//  "Optional" Forty Billion loops just to get one additional digit, the tenth digit of pi 
        ...
        if iterInt64 == 2000000000 {
            p("  ... working ... on another Billion iterations ... working ...")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
        if iterInt64 == 3000000000 {
            p("  ... working ... on another Billion iterations ... working ... Ctrl-C to End/Exit")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
        ...
        if iterInt64 == 9000000000 {
            p("  ... working ... on another five Billion iterations ... working ...")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
        if iterInt64 == 9500000000 {
            p("  ... working ... on another five Billion iterations ... working ...")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
        if iterInt64 == 10000000000 {
            p("  ... working ... on another five Billion iterations ... working ...")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
        if iterInt64 == 15000000000 {
            p("  ... working ... on another ten Billion iterations ... working ...")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
        if iterInt64 == 25000000000 {
            p("  ... working ... on another ten Billion iterations ... working ...")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
        if iterInt64 == 35000000000 {
            p("  ... working ... just another five Billion iterations to go ... working ... ")
                t := time.Now()
                elapsed := t.Sub(start1)
            p(elapsed)
        }
...
fmt.Println("If the interest is credited twice in the year, the interest rate for each 6 months will be 50%, so ")
fmt.Println("the initial $1 is multiplied by 1.5 twice, yielding $1.00 × 1.52 = $2.25 at the end of the year. ")
fmt.Println("Compounding quarterly yields $1.00 × 1.254 = $2.44140625, and compounding monthly yields : ")
fmt.Println("$1.00 × (1 + 1/12)12 = $2.613035.... If there are n compounding intervals, the interest for each")
fmt.Println("interval will be 100%/n and the value at the end of the year will be $1.00 × (1 + 1/n)^n.")
//
