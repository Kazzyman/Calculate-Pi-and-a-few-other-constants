...
//fmt.Print("Enter your selection from above, 1 - 10 \u00a9 2022, by Richard Hart Woolley\n")
// the above commented line is definately not needed in a Linux environment
fmt.Println("Ctrl-C to End/Exit  SLOC including comments and white space = 1440 \n")
}
...
if num > 14 && num < 100 { num = 15 }  // to display a funny out-of-range message as case 15:
...
case 10:
    ...
    var index = 0 
        ...
        fmt.Println("bottom times 3 is ", smallerPerfectSquareOnce*3, "  Total Iterations completed was ", TotalIterations)
        fmt.Println("very close to     ", Prospective_smaller_hit, "\n")
                t10 := time.Now()
...
var smallerPerfectSquareOnce int  // global vars will be used in multiple functions (func)s , and in case: 10 
var savedHit int 
...
// should instead be deciding final answer based on closeness of fit, percent diff, to exactly 3x smaller perfect square. Not just checking to see if we have gotten this close to the known value of Sqrt of 3
                    ...
                    sr_per_lg := (math.Sqrt(float64(ProspectiveHitOnLargeSide)) / math.Sqrt(float64(smallerPerfectSquareOnce)))
                            if sr_per_lg > 1.7320508066 { if sr_per_lg < 1.7320508074 { fmt.Println(sr_per_lg, "from bottom, is close at", iter, "iterations, hits are:", ProspectiveHitOnLargeSide, smallerPerfectSquareOnce)} } 
            ...
            }  // end if loop "if we have found a candidate" we have already dealt with it so we need to break and get another potential from the for loop ???
