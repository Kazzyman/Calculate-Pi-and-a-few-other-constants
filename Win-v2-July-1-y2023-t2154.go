...
func main() {       // top-level program logic flow -- explore SEVENTEEN ways to calculate Pi, plus THREE other constants
    for 1 == 1 {
        totalLines, nonEmptyLines, err, filenameOfMe := reportSLOCstats()  // this all needs to be removed for release versions --AMFmain01a
            if err != nil {                                 //
                log.Fatal(err)                              //
            }                                               // --AMFmain01b
            // totalLines := nnnnn                     // and these two UNcommented
            // nonEmptyLines := nnnnnn                 // 
        DisplayMenus(totalLines, nonEmptyLines, filenameOfMe) 
...
func reportSLOCstats() (totalLines, nonEmptyLines int, err error, myFileName string) {
    ...
    file, err := os.Open(filename)              // find this source code file 
    if err != nil {
        return 0, 0, err, ""
    }
    defer file.Close()
...
fmt.Print("Enter your selection, 1 -> x", string(colorRed), " (IS THIS WINDOW MAXIMIZED?  Well, do it!)\n", string(colorReset)) 
    ...
    switch selection { // if selection == 2 then handel per case 2: etc. 
        ...
        case 57:
            showTheMagicBehind_Gauss_Legendre()
...
func showTheMagicBehind_Gauss_Legendre(){ // case 57: 
    var Legendre_rune = `
