...
import (
    ...
    var pathNameOfMeAsExe = os.Args[0]                         // the objective is to find this source code file 
    var re = regexp.MustCompile(`\\exe\\(.*?)\.exe`) 
    var match = re.FindStringSubmatch(pathNameOfMeAsExe) // grab the stuff between /exe/ and .exe in the full pathname of the generated executable 
    var filenameOfThisFile = "a string"

func main() {       // top-level program logic flow -- explore SEVENTEEN ways to calculate Pi, plus THREE other constants
    getTheNameOfThisSourceCodeFile()
    ...
    for 1 == 1 {
        DisplayMenus(totalLines, nonEmptyLines, filenameOfThisFile) 
...
func getTheNameOfThisSourceCodeFile() {
    ...
    var substring string 
        ...
        filenameOfThisFile = substring + ".go"                // append the .go on the end of it 
}
...
// This next func is extremely long, contains all menus, and a very long switch statement 
func DisplayMenus(totalLines, nonEmptyLines int, filenameOfMe string) {
    pages_of_code := totalLines / 49 
    var selection int 
    //selection = 0 // a switching flag? in the unix version ???
...
fmt.Println(string(colorYellow), 
  ...
  string(colorGreen), "   build date :", string(colorCyan), date.Format("(01-02-2006)"), string(colorReset), filenameOfMe, "\n")
...
fmt.Print("Enter your selection, 1 -> x", string(colorRed), " (IS THIS WINDOW MAXIMIZED?  Well, do it!)\n", string(colorReset)) 
    ...
    switch selection { // if selection == 2 then handel per case 2: etc. 
        ...
        case 37:
            ...
            case 57:
                //showTheMagicBehind_Gauss_Legendre() // for compiled release versions 
                ShowGauss_Legendre()
            case 557:
                showTheMagicBehind_Gauss_Legendre() // for compiled release versions 
...
func TheSpigot(selection int){  // case 19: 
    usingBigFloats = false
...
// used in func BBPF(selection int) // case 2:
...
// used in chud(selection int) {  // case 15:
// calculate Pi for n number of digits
//func CalcPi(digits float64, start time.Time, precR uint, loops int, selection int) (*big.Float, uint, int) {
...
queryIfTimeToDie := 1 // this is a flag; if it is set to zero we exit 
...
func compareFloat64withBigFloats() {
    usingBigFloats = true
...
func ShowGauss_Legendre() { // case 57: 
    ...
    line := "some text just to create the line var"  // Create a var to store the text of one line 
...
// /*
func showTheMagicBehind_Gauss_Legendre(){ // case 57: // for compiled release versions 
    var Legendre_rune = `
// Gauss–Legendre algorithm in Go: Because of how float64 is implemented in Go, this only gives 15 correct digits of Pi
// Project: do a version that uses big Floats 
// Authorship: juuso@mit.edu , and further modified by rick.woolley@gmail.com 

The Gauss–Legendre algorithm is notable for being rapidly convergent, with only 25 iterations producing 45 million correct digits of π.

The algorithm works by repeatedly replacing two numbers by their arithmetic and geometric mean, in order to approximate their arithmetic-geometric mean. 
...
Richard Brent and Eugene Salamin.

The algorithm starts with two numbers, a and b, which are initialized to 1 and 1/√2, respectively. The arithmetic-geometric mean of a and b is then 
calculated, and the two numbers are replaced by their arithmetic-geometric mean. This process is repeated until the desired accuracy is reached.

The following is the pseudocode for the Gauss–Legendre algorithm:

function gauss_legendre(a, b, t, n):
    if n = 0:
        return a, b, t
    else:
        g = (a + b) / 2
        m = sqrt(b * g)
        t = t - (2 ** n) * (g - a) ** 2
        return gauss_legendre(g, m, t, n - 1)


The algorithm is named after Carl Friedrich Gauss and Adrien-Marie Legendre, who independently developed the theory of Legendre polynomials, which are 
used in the algorithm.

The Gauss–Legendre algorithm is a very efficient algorithm for computing the digits of π. However, it has some drawbacks, such as the fact that it is 
computer memory-intensive. Therefore, all record-breaking calculations for many years have used other methods, almost always the Chudnovsky algorithm.
