...
import (
    ...
    "log"          // Unix fix required
    "strings"     // Unix fix required
)
...
func main() {       // top-level program logic flow -- explore SEVENTEEN ways to calculate Pi, plus THREE other constants --AMFmain-A-b
    ...
    //
    // /* Unix variant
        re2 := regexp.MustCompile(`Unix-(.+)\.go`) // Unix fix required, matched nada    // Unix fix required
    // Unix variant */
    //
    /* Windows variant
    ...
    for 1 == 1 {
        ...
        // /* Unix variant
            fmt.Scanf("%d", &JustToPauseTheLoop) // must pause the endless loop after every switch case to prevent menu redisplay over results
                //fmt.Scan(&JustToPauseTheLoop) // Scan() works as a pause, but requies data input to continue 
        // Unix variant */ 
        //
        /* Windows variant 
            fmt.Scanf("%d", &JustToPauseTheLoop) // must pause the endless loop after every switch case to prevent menu redisplay over results 
...
// Windows variant */
    ...
    var substring string 
        if len(match) >= 2 {
            substring = match[0]                   // substring is loaded with our base file name 
        } else {
            fmt.Println("Substring not found in getTheNameOfThisSourceCodeFile")
        }
        //fmt.Println("substring is: ", substring) // substring is empty ?
