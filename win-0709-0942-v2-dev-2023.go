...
import (
    ...
    "bufio"         // used to count the lines in this file for reportSLOCstats()
    "log"          
    "strings"    
)

// convenience globals: 
    var SansVerOfNameOfThisFile = "string"      // this var is for a string 
    var usingBigFloats = false 
    var filenameOfThisFile = "another string"   // this var is for a string


func main() {       // top-level program logic flow -- explore SEVENTEEN ways to calculate Pi, plus THREE other constants --AMFmain-A-b
    getTheNameOfThisSourceCodeFile()  // for vvvvvvv and for Menu header code, five lines below (load into global var filenameOfThisFile)
    ...
    match2 := re2.FindStringSubmatch(filenameOfThisFile) // grab the stuff between Win- and .go in the name of this file 
        var substring2 string 
        if len(match2) >= 2 {

            /* Unix variant 
                substring2 = match2[0]                   // Unix substring is loaded with our base file name 
            Unix variant */ 
            //
            // /* Windows variant 
                substring2 = match2[1]                   // Windows substring is loaded with our base file name 
    // why oh why is it crashing here ???
    // it does ... ... ^ right there 
    // and not here
    // or here
    // variant * / 
    // or even up thar ?? 
            ...
            fmt.Println("Substring2 not found")
        }
        SansVerOfNameOfThisFile = substring2
...
func getTheNameOfThisSourceCodeFile() {
    // get the name of this sourceCode file to be used in the display of source code funcs 
...
func reportSLOCstats() (totalLines, nonEmptyLines int, err error) {   // --AMFreportSLOCstats01a
    ...
    return totalLines, nonEmptyLines, nil            // substring is called/named myFileName in the func definiiton, which is ok 
...
fmt.Println("995: create a refactored source code file for Windows to Unix, or Unix to Windows variants \n\n")
...
fmt.Print("Enter your selection, 1 -> x", string(colorRed), " (IS THIS WINDOW MAXIMIZED?  Well, do it!)\n", string(colorReset)) 
    ...
    switch selection { // if selection == 2 then handel per case 2: etc. 
        ...
        case 999:
            ...
            // /* Windows variant 
                    case 995:
                        refactorWindowsToUnix()
            // Windows variant */ 
            //
            /* Unix variant 
                    case 995:
                        refactorUnixToWindows()
            Unix variant */ 
...
// case 180:  /// this procedure is really only meant to be used for testing or illustration (calc root of all integers; build a file of all results)
// -- AMFxroyCCA
...
// Rick loves github.com, Sublime Text, and Go.lang 
...
func printResultStatsLong(sumBig *big.Float, precision int, useAlternateFile string, arg01 int, TotalRun string, selection int) {  // prslc2d==default, prslc2c==chud
    ...
    // now, in any instance, we add comments to the default log file if an alternate file has been used ... 
...
func bogusKluge() {
            ...
            //
            // /* Windows variant 
func refactorWindowsToUnix() {
            // Windows variant */ 


    nameOfOldSourceCodeFile := filenameOfThisFile

    regexp2stripDotGo := regexp.MustCompile(`win-(.+)\.go`)
        ...
        for scanner.Scan() {
            lineRead := scanner.Text() // this fixes my main problem re formatting getting wasted 
                if scanner.Err() != nil {
                    ...
                    regexUnixEnd            := regexp.MustCompile(` Unix variant \*\/ `)               // ... Unix var asterisk/ 

            // write the read line unchanged to the output file only if none of these four strings are found in the read line from the input file
            // when any one or more of these do match we skip echoing the line and proceed to find exactly which one did match, and then we change it 
...
// replace1-followingLine
            ...
            if regexUnixEnd.MatchString(lineRead) {
                // write the edited line to the output file
                OutputFileHandle, err45 := os.OpenFile(nameOfNewSourceCodeFile, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600) // write to file 
                check(err45)                               // ... gets a file handle for output 
                defer OutputFileHandle.Close()          // It’s idiomatic to defer a Close immediately after opening a file.
// replace2-followingLine
                    _, err56 := fmt.Fprint(OutputFileHandle, "            // Unix variant */ \n ")  // dealing with a rouge asterisk/ here *** why/how do we get away with it ?? ***
                    check(err56)
            }             
            ...
            if regexWindowsStart.MatchString(lineRead) {
                // write the edited line to the output file
                OutputFileHandle, err46 := os.OpenFile(nameOfNewSourceCodeFile, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600) // write to file 
                check(err46)                               // ... gets a file handle for output 
                defer OutputFileHandle.Close()          // It’s idiomatic to defer a Close immediately after opening a file.
// this next line does not *** USED TO NOT *** get clobbered 
                    _, err57 := fmt.Fprint(OutputFileHandle, "            /* Windows variant \n ")  // dealing with a rouge /asterisk here *** why/how do we get away with it ?? ***
                    check(err57)
            } 
            if regexWindowsEnd.MatchString(lineRead) {
                // write the edited line to the output file
                OutputFileHandle, err47 := os.OpenFile(nameOfNewSourceCodeFile, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600) // write to file 
                check(err47)                               // ... gets a file handle for output 
                defer OutputFileHandle.Close()          // It’s idiomatic to defer a Close immediately after opening a file.
// I am esspecially puzzeled that the following line SOMETIMES does NOT get clobberd since it contains precisely the same ... variant asterisk/\n as exists in one of the two lines that do get clobbered???
...
func bogusGluge() {
    ...
    nameOfOldSourceCodeFile := filenameOfThisFile
        ...
        regexp2stripDotGo := regexp.MustCompile(`Unix-(.+)\.go`) 

    match2 := regexp2stripDotGo.FindStringSubmatch(filenameOfThisFile) // grab the stuff between Unix- and .go in the name of this file 
        var substring string 
        if len(match2) >= 2 {
            substring = match2[0]                   // Unix substring is loaded with our base file name 
        } else {
            fmt.Println("Substring2 not found in refactorUnixToWindows")
        }
        filenameOfThisFileSansDotGo := substring


    nameOfNewSourceCodeFile := filenameOfThisFileSansDotGo + "-now-Windows.go"
    ...
    defer fileHandle.Close()
        ...
        for scanner.Scan() {
            lineRead := scanner.Text() // this fixes my main problem re formatting getting wasted 
                                                                                                    // this is Unix to Windows, so: 
                    regexWindowsStart       := regexp.MustCompile(`\s* \/\* \s*Windows\s*variant\s*`)            // ... /asterisk Windows variant  // bug fixed
                    regexWindowsEnd         := regexp.MustCompile(`\s*Windows\s*variant\s* \*\/\s*`)            // ... Windows variant asterisk/
                    regexUnixStart          := regexp.MustCompile(`\s*\/\/ \s* \/\*\ s*Unix\s*variant\s*`)        // ... // /asterisk Unix variant
                    regexUnixEnd            := regexp.MustCompile(`\s*\/\/ \s*Unix\s*variant\s* \*\/\s*`)        // ... // Unix variant asterisk/  
            ...
            if regexWindowsStart.MatchString(lineRead) {
                ...
                defer OutputFileHandle.Close()          // It’s idiomatic to defer a Close immediately after opening a file.
// only this line does not (used to not) get clobbered ???
            ...
            if regexUnixEnd.MatchString(lineRead) {
                ...
                defer OutputFileHandle.Close()          // It’s idiomatic to defer a Close immediately after opening a file. 
                    _, err5 := fmt.Fprint(OutputFileHandle, "            Unix variant */ \n ") // dealing with a rouge asterisk/ here *** why/how do we get away with it ?? ***
                    check(err5)
            } // have yet to make a show func for deleteAllLogFiles() // case 999: *************************************************************************************************
        }
    if scanner.Err() != nil {
