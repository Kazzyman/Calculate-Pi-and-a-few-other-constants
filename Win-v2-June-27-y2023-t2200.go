...
if loops < 100 {
    ...
    fmt.Println("... but : ")
    printResultStatsLong(pi, int(digits), "ChudDidLessThanOneHundredLoops")
}
...
// used in func BBPF(selection int) // case 2:
// and in func chud CalcPi // case 15: 
func printResultStatsLong(sumBig *big.Float, precision int, useAlternateFile string) {  // prslc2d==default, prslc2c==chud
    ...
    // *************************************** this is the one and only logging loop ******************************************************************************        
        ...
        if useAlternateFile == "chudDid800orMoreLoops" {
            ...
            _, err2prslc2c := fmt.Fprintf(fileHandleNilakan, "\nThese are the %d verified digits we have calculated via Nilakantha: \n", copyOfLastPosition)
                check(err2prslc2c)
                    for _, oneChar := range stringVerOfCorrectDigits {
                        fmt.Print(oneChar) // to the console  
