...
func main() {       // top-level program logic flow -- explore SEVENTEEN ways to calculate Pi, plus THREE other constants
    for 1 == 1 {
        totalLines, nonEmptyLines, err := reportSLOCstats()  // this all needs to be removed for release versions 
            if err != nil {                                 //
                log.Fatal(err)                              //
            }                                               // 
...
func DisplayMenus(totalLines, nonEmptyLines int) {
    ...
    var selection int 
    selection = 0 // a switching flag? But also definately the scanned value being reset ?
...
/*
// used in chud(selection int) {  // case 15: // via : 
// .. CalcPi(digits float64, start time.Time, precR uint, loops int, selection int) (*big.Float, uint, int) {
// need to pass in selection int and i

func logChudData(selection int, i int, start time.Time, pi big.Float, digits float64) {
    t := time.Now()
    elapsed := t.Sub(start)
    // only if 
        if int(elapsed.Seconds()) != 0 {
            ...
            var peakAtDigits int 
            if i >= 50 && i <= 100 { 
                peakAtDigits = 75 
                _ , err8 := fmt.Fprintf(fileHandle, "a peak %1.[1]*[2]f \n", peakAtDigits, pi) 
                    check(err8)
            }
            if i > 100 && i <= 200 { 
                peakAtDigits = 85 
...
precR, loops = CalcPi(float64(digits), start, precR, loops, selection)
...
func CalcPi(digits float64, start time.Time, precR uint, loops int, selection int) (uint, int) {
    var i int 
    /**
    ...
    */
    ...
    //n := int64(2 + int(float64(digits)/12))
...
test := 1 // this is a flag; if it is set to zero we exit 
    ...
    for ; n > 0; n-- {
        ...
        if i == 100 {
            ...
            fmt.Scanf("%d", &test) 
                ...
                //logChudData(selection, i, start, *pi, digits) // case#, iterations, startTime, pi, requestedDigits 

                if test == 0 {
                    fmt.Println("test is 0, time to die")
                    //printResultStatsLong(pi, int(digits), useAlternateFile) 
                    //logChudData(selection, i, start, *pi, digits)

                //fmt.Printf("%.999f", pi)
                fmt.Printf("\nprecR is: %d, and precision is: %d", precR, prec)
                break
                //os.Exit(1)
                }
        }
        if i == 200 {
            fmt.Println("\n we are at 200 loops \n")
            fmt.Printf("%.999f", pi)
            fmt.Println("\n we are at 200 loops, here comes a 999f of pi as a big float: \n")
            fmt.Println("enter any int to continue, 0 to end")
            fmt.Scanf("%d", &test) 
            fmt.Scanf("%d", &test) 
            printResultStatsLong(pi, int(digits), useAlternateFile) 
        ...
        if i == 400 {
            fmt.Println("\n we are at 400 loops, here comes a 8999f of pi as a big float:  \n")
            fmt.Printf("%.8999f", pi)
            fmt.Println("\n we are at 400 loops \n")
            fmt.Println("enter any int to continue, 0 to end")
            fmt.Scanf("%d", &test) 
        ...
        if i == 800 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 800 loops, here comes a 9999f of pi as a big float:  \n")
            fmt.Printf("%.9999f", pi)
            fmt.Println("\n we are at 800 loops \n")
            fmt.Println("enter any int to continue, 0 to end")
            fmt.Scanf("%d", &test) 
        ...
        if i == 1600 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 1600 loops, here comes a 11000f of pi as a big float:  \n")
            fmt.Printf("%.11000f", pi)
        ...
        if i == 2000 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 2000 loops, here comes a 12000f of pi as a big float:   \n")
            fmt.Printf("%.12000f", pi)
        ...
        if i == 2400 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 2400 loops, here comes a 13000f of pi as a big float:   \n")
            fmt.Printf("%.13000f", pi)
        ...
        if i == 2800 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 2800 loops, here comes a 14000f of pi as a big float:   \n")
            fmt.Printf("%.14000f", pi)
        ...
        if i == 3200 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 3200 loops, here comes a 17999f of pi as a big float:   \n")
            fmt.Printf("%.17999f", pi)
        ...
        if i == 4000 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 4000 loops, here comes a 19999f of pi as a big float:    \n")
            fmt.Printf("%.19999f", pi)
        ...
        if i == 6000 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 6000 loops, here comes a 20599f of pi as a big float:    \n")
            fmt.Printf("%.20599f", pi)
            fmt.Println("\n we are at 4000 loops \n")
            fmt.Println("enter any int to continue, 0 to end")
            fmt.Scanf("%d", &test) 
            fmt.Scanf("%d", &test) 
        ...
        if i == 8000 {
            useAlternateFile = "chud"
            fmt.Println("\n we are at 8000 loops, here comes a 22999f of pi as a big float:    \n")
            fmt.Printf("%.22999f", pi)
            fmt.Println("\n we are at 4000 loops \n")
            fmt.Println("enter any int to continue, 0 to end")
            fmt.Scanf("%d", &test) 
            fmt.Scanf("%d", &test)
...
// used in func BBPF(selection int) // case 2:
func printResultStatsLong(sumBig *big.Float, precision int, useAlternateFile string) {  // prslc2d==default, prslc2c==chud
    ...
    stringOfSum := sumBig.Text('f', 999907) // create a string version of a big result, up to a million digits can be verified with this  
    ...
    fmt.Printf("\nWe have calculated pi correctly to %d digits using precision of %d \n", copyOfLastPosition, precision)
