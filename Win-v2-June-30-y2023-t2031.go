...
func main() {       // top-level program logic flow -- explore SEVENTEEN ways to calculate Pi, plus THREE other constants
    for 1 == 1 {
        totalLines, nonEmptyLines, err, filenameOfMe := reportSLOCstats()  // this all needs to be removed for release versions 
            if err != nil {                                 //
                log.Fatal(err)                              //
            }                                               // 
...
fmt.Println("14:", string(colorCyan), "Pi:", string(colorReset), "modified Archimedes' method using Go's math/big objects: 3,012 digits")
...
fmt.Println("        Four-Billion iterations will be executed .... using float64 types")
...
fmt.Println("        Three-Hundred-Million iterations will be executed .... float64 types")
...
fmt.Println("212:   Read stats from prior runs of ALL cases \n")

fmt.Println("202:   Read stats from prior runs of BBPF (case 2:) \n") 

fmt.Println("214:   Read stats from prior runs of modified Archimedes' method (case 14:) \n")

fmt.Println("215:   Read stats from prior runs of the Chudnovsky method (case 15:) \n")

fmt.Println("205:   Read stats from prior runs of Nilakantha Somayaji (case 5:) \n")


fmt.Println("241:   Read stats from prior runs of Bailey–Borwein–Plouffe formula [concurent] (case 41:) \n")

fmt.Println("315:   Look at the really big pie that came out of the Chudnovsky method (case 15:) \n")


fmt.Println("180: run the endless test of case 18 \n")
...
i = 1 // a secondary dedicated loop counter
    ...
    for ; n > 0; n-- {
        ...
        if i == 100 {
            ...
            fmt.Scanf("%d", &test) 
            fmt.Scanf("%d", &test) // Windows environments seem to just fly past a single Scanf, so I have used two :) 
        }
        if i == 200 {
        ...
        if test == 0 {
                fmt.Println("test is 0 (outside the other ifs), time to die")
                fmt.Printf("\nprecision was: %d \n", prec)
                break
        }
        // 1,000,000 digits requires 70516 loops, per the run on May 7 2023 at 10:30
    ...
    // we are out of the loop, so we do the following just once:
            ...
            _ , err9bigpie := fmt.Fprint(fileHandleBig, pi) // dump this big-assed pie to a txt file
                check(err9bigpie)
...
func printResultStatsLong(sumBig *big.Float, precision int, useAlternateFile string, arg01 int, TotalRun string, selection int) {  // prslc2d==default, prslc2c==chud
    ...
    if copyOfLastPosition > 55000 { // we have something really big
...
// sumBig *big.Float, precision int, useAlternateFile string, arg01 int, TotalRun string
...
// LOG TO THE MASTER LOGGING FILE :: dataLog-From_calculate-pi-and-friends.txt  ================================================================================
        ...
        if useAlternateFile == "chudDid800orMoreLoops" {
            ...
            fileHandleDefault, err1prslc2d := os.OpenFile("dataLog-From_calculate-pi-and-friends.txt", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600) // append to file 
                check(err1prslc2d)                                // ... gets a file handle to dataLog-From_calculate-pi-and-friends.txt
                defer fileHandleDefault.Close()                  // It’s idiomatic to defer a Close immediately after opening a file.
            _, err2prslc2da := fmt.Fprint(fileHandleDefault, "\nStats from running BBPF (case 2:) can be viewed in dataLog-From_BBPF_Method_lengthy_prints.txt\n")
                check(err2prslc2da)
            fileHandleDefault.Close() 
            ...
            fileHandleDefault, err1prslc2d := os.OpenFile("dataLog-From_calculate-pi-and-friends.txt", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600) // append to file 
                check(err1prslc2d)                                // ... gets a file handle to dataLog-From_calculate-pi-and-friends.txt
                defer fileHandleDefault.Close()                  // It’s idiomatic to defer a Close immediately after opening a file.
            _, err2prslc2da := fmt.Fprint(fileHandleDefault, "\nStats from running BBPfConcurent (case 41:) can be viewed in dataLog-From_BBPfConcurent.txt\n")
                check(err2prslc2da)
            fileHandleDefault.Close() 
...
// sumBig *big.Float, precision int, useAlternateFile string, arg01 int, TotalRun string
            ...
            _ , err7 := fmt.Fprintf(fileHandleDefault, "Total run was %s \n ", TotalRun) 
                check(err7)
            _, err2prslc2da := fmt.Fprint(fileHandleDefault, "\nStats from running Nilakantha (case 5:) can be viewed in dataLog-From_Nilakantha_Method_lengthy_prints.txt\n")
                check(err2prslc2da)
            fileHandleDefault.Close() 
        }
...
// store reults in a log file which can be displayed from within the program by selecting option #12
            ...
            _ , err5 := fmt.Fprintf(fileHandle, "%d was total Iterations; %d was precision setting for the big.Float types \n", iters, precision)
                check(err5)
            TotalRun := elapsed.String() // cast time durations to a String type for Fprintf "formatted print"
            _ , err7 := fmt.Fprintf(fileHandle, "Total run was %s \n ", TotalRun) 
                check(err7)
