...
// convenience globals: 
    var SansVerOfNameOfThisFile = "string"
    var usingBigFloats = false 
    var filenameOfThisFile = "as a string"

        /* Unix variant // // // from v1 (most of which is probably obsolete, except the single fmt.Scanf)
...
func main() {       // top-level program logic flow -- explore SEVENTEEN ways to calculate Pi, plus THREE other constants --AMFmain-A-b
    ...
    // for menu header data generation (inception: stripped file name)
    ...
    match2 := re2.FindStringSubmatch(filenameOfThisFile) // grab the stuff between Win- and .go in the name of this file 
        ...
        if len(match2) >= 2 {
            substring2 = match2[1]                   // substring is loaded with our base file name 
        } else {
...
// This next func is extremely long, contains all menus, and a very long switch statement -- AMFmenusA
func DisplayMenus(totalLines, nonEmptyLines int) {

/* Unix variant
        num = 0  // probable obsolete unix variant 
Unix variant */
...
/* Unix variant
    selection = 0 // a switching flag? in the unix version ??? // probably safe to assume is obsolete in v2
Unix variant */ 
...
/* Unix variant
    fmt.Scanf("%d", &selection)  // pause and request input from the user // Scan does not work the same here as does Scanf 
Unix variant */  
...
// had the following here in v1: // // // (all of this should probably safe to assume is obsolete?)

        /* Unix variant
            universal_switch(1) // want to get away from this in v2
        Unix variant */ 

        // /* Windows variant (v1)
            // // // universal_switch() // at the end of DisplayFirstMenu() we call universal_switch() which will display the second menu ONLY if num == 13 
                              // ... otherwise it handles what WAS caught in num that WAS NOT == 13 
        // Windows variant */ 
        ...
        // /* Windows variant
        // // // func universal_switch() {
            // // // if num == 13 { // // // // then , if, does second menu fmt.println's etc. 
        // Windows variant */ 
...
// uni and win var should be same:
